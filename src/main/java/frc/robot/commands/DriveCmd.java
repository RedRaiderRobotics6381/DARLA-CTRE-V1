...
public class DriveCmd extends Command {
  ...
  public void execute() {
    if(RunElevator.amp && Robot.robotContainer.s_Intake.botFull()){
      ...
      m_DriveTrain.setControl(swerveRequestFacing.withVelocityX(translationSup.getAsDouble())
      ...
      if(!Robot.robotContainer.s_Intake.botFull()){
        if(s_Vision.getNoteTargets()){
          if(robotCentricSup.getAsBoolean()){
            m_DriveTrain.setControl(robotCentric.withVelocityX(translationSup.getAsDouble())
                                            .withVelocityY(strafeSup.getAsDouble())
                                            .withRotationalRate(s_Vision.getNoteRotationSpeed()));
          }
          else{
            m_DriveTrain.setControl(fieldCentric.withVelocityX(translationSup.getAsDouble())
                                            .withVelocityY(strafeSup.getAsDouble())
                                            .withRotationalRate(s_Vision.getNoteRotationSpeed()));
          }
        }
